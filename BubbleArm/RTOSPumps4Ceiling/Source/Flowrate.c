...
volatile int32_t Flowrate[8];
volatile uint32_t count[8];
volatile uint32_t rate[8][2];
...
void FlowrateInit()
{
	...
	//setup timer
	LPC_SYSCON ->SYSAHBCLKCTRL |= (1 << 9);  //enable clock for 32bit timer0
	LPC_SYSCON ->SYSAHBCLKCTRL |= (1 << 10); //enable clock for 32bit timer1
...
void TIMER32_1_IRQHandler(void)
{
	...
	if (LPC_TMR32B1 ->IR & 0x01)
	{
		...
		if(Flowrate[0]>Flowrate[2] || Flowrate[1]>Flowrate[2])
		{
		...
		if(Flowrate[3]>Flowrate[5] || Flowrate[4]>Flowrate[5])
		{
			if(Flowrate[3]>Flowrate[4])
				Flowrate[4] = -Flowrate[4];
			else
				Flowrate[3] = -Flowrate[3];
		}
	}
