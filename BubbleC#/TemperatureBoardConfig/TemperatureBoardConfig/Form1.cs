...
namespace TemperatureBoardConfig
{
    public partial class Form1 : Form
    {
        ...
        private void comboBoxT_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            int index = TempBoards.IndexOf(CurTempNodID);
            int[] config = ((TemperatueBoard)TempBoards[index]).GetConfig();
            ...
            textBoxCFG9.Text = config[8].ToString();
            textBoxCFG10.Text = config[9].ToString();
            textBoxCFG11.Text = config[10].ToString();
            textBoxCFG12.Text = config[11].ToString();
            textBoxCFG13.Text = config[12].ToString();
            textBoxCFG14.Text = config[13].ToString();
            textBoxCFG15.Text = config[14].ToString();
            textBoxCFG16.Text = config[15].ToString();
        }
        ...
        private void buttonSave_Click(object sender, EventArgs e)
        {
            ...
            string dir = Directory.GetCurrentDirectory() + "\\Config\\Temperature.config";
            fs = new FileStream(dir, fileMode, FileAccess.Write);
            sw = new StreamWriter(fs, Encoding.Default);

            string content;
            content = "#this file is the configuration file for the temperature boards. Each line is a configuration for one board.\r\n";
            content += "#Each line should follow the following format:\r\n";
            content += "#ID:CH1,CH2,CH3,CH4,CH5,CH6,CH7,CH8,CH9,CH10,CH11,CH12,CH13,CH14,CH15,CH16\r\n";
            content += "#the ID is the ID of the temperature board. Right now we got two boards with IDs 1 and 2. The CH1 to CH16 rep-\r\n";
            content += "#resent the sensor configuration information connected to coresponding chanel.\r\n\r\n";

            foreach (object o in Temperatureconfig)
            {
                if(TempBoards.Contains( ((TemperatureConfig)o).id ) == false )
                {
                    content += ((TemperatureConfig)o).id.ToString() + ":";
                    content += string.Format("{0},{1},{2},{3},",((TemperatureConfig)o).config[0],((TemperatureConfig)o).config[1],((TemperatureConfig)o).config[2],((TemperatureConfig)o).config[3]);
                    content += string.Format("{0},{1},{2},{3},",((TemperatureConfig)o).config[4],((TemperatureConfig)o).config[5],((TemperatureConfig)o).config[6],((TemperatureConfig)o).config[7]);
                    content += string.Format("{0},{1},{2},{3},",((TemperatureConfig)o).config[8],((TemperatureConfig)o).config[9],((TemperatureConfig)o).config[10],((TemperatureConfig)o).config[11]);
                    content += string.Format("{0},{1},{2},{3}",((TemperatureConfig)o).config[12],((TemperatureConfig)o).config[13],((TemperatureConfig)o).config[14],((TemperatureConfig)o).config[15]);
                    content += "\r\n";
                }
            }
            foreach (object o in TempBoards)
            {
                content += ((TemperatueBoard)o).ID.ToString() + ":";
                int[] config = ((TemperatueBoard)o).GetConfig();
                ...
                content += string.Format("{0},{1},{2},{3},", config[8], config[9], config[10], config[11]);
                content += string.Format("{0},{1},{2},{3}", config[12], config[13], config[14], config[15]);
                content += "\r\n";
            }
        ...
        private void textBoxCFG1_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            {
                if (TempBoards.Contains(CurTempNodID) == true)
                {
                    try
                    {
                        int i = int.Parse(textBoxCFG1.Text);
                        int index = TempBoards.IndexOf(CurTempNodID);
                        ((TemperatueBoard)TempBoards[index]).SetConfig(1, i);
                    }
                    catch
                    {
                        MessageBox.Show("Error: wrong input. Please input an integer!");
                        int index = TempBoards.IndexOf(CurTempNodID);
                        int[] config = ((TemperatueBoard)TempBoards[index]).GetConfig();
                        textBoxCFG1.Text = config[0].ToString();
                    }
            ...
            if (e.KeyCode == Keys.Enter)
            {
                if (TempBoards.Contains(CurTempNodID) == true)
                {
                    try
                    {
                        ...
                        int index = TempBoards.IndexOf(CurTempNodID);
                        ((TemperatueBoard)TempBoards[index]).SetConfig(2, i);
                    }
                    catch
                    {
                        MessageBox.Show("Error: wrong input. Please input an integer!");
            ...
            if (e.KeyCode == Keys.Enter)
            {
                if (TempBoards.Contains(CurTempNodID) == true)
                {
                    try
                    {
                        ...
                        int index = TempBoards.IndexOf(CurTempNodID);
                        ((TemperatueBoard)TempBoards[index]).SetConfig(3, i);
                    }
                    catch
                    {
                        MessageBox.Show("Error: wrong input. Please input an integer!");
            ...
            if (e.KeyCode == Keys.Enter)
            {
                if (TempBoards.Contains(CurTempNodID) == true)
                {
                    try
                    {
                        ...
                        int index = TempBoards.IndexOf(CurTempNodID);
                        ((TemperatueBoard)TempBoards[index]).SetConfig(4, i);
                    }
                    catch
                    {
                        MessageBox.Show("Error: wrong input. Please input an integer!");
            ...
            if (e.KeyCode == Keys.Enter)
            {
                if (TempBoards.Contains(CurTempNodID) == true)
                {
                    try
                    {
                        ...
                        int index = TempBoards.IndexOf(CurTempNodID);
                        ((TemperatueBoard)TempBoards[index]).SetConfig(5, i);
                    }
                    catch
                    {
                        MessageBox.Show("Error: wrong input. Please input an integer!");
            ...
            if (e.KeyCode == Keys.Enter)
            {
                if (TempBoards.Contains(CurTempNodID) == true)
                {
                    try
                    {
                        ...
                        int index = TempBoards.IndexOf(CurTempNodID);
                        ((TemperatueBoard)TempBoards[index]).SetConfig(6, i);
                    }
                    catch
                    {
                        MessageBox.Show("Error: wrong input. Please input an integer!");
            ...
            if (e.KeyCode == Keys.Enter)
            {
                if (TempBoards.Contains(CurTempNodID) == true)
                {
                    try
                    {
                        ...
                        int index = TempBoards.IndexOf(CurTempNodID);
                        ((TemperatueBoard)TempBoards[index]).SetConfig(7, i);
                    }
                    catch
                    {
                        MessageBox.Show("Error: wrong input. Please input an integer!");
            ...
            if (e.KeyCode == Keys.Enter)
            {
                if (TempBoards.Contains(CurTempNodID) == true)
                {
                    try
                    {
                        ...
                        int index = TempBoards.IndexOf(CurTempNodID);
                        ((TemperatueBoard)TempBoards[index]).SetConfig(8, i);
                    }
                    catch
                    {
                        MessageBox.Show("Error: wrong input. Please input an integer!");
        ...
        private void textBoxCFG9_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                if (TempBoards.Contains(CurTempNodID) == true)
                {
                    try
                    {
                        int i = int.Parse(textBoxCFG9.Text);
                        int index = TempBoards.IndexOf(CurTempNodID);
                        ((TemperatueBoard)TempBoards[index]).SetConfig(9, i);
                    }
                    catch
                    {
                        MessageBox.Show("Error: wrong input. Please input an integer!");
                        int index = TempBoards.IndexOf(CurTempNodID);
                        int[] config = ((TemperatueBoard)TempBoards[index]).GetConfig();
                        textBoxCFG1.Text = config[8].ToString();
                    }
                }
            }
        }

        private void textBoxCFG10_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                if (TempBoards.Contains(CurTempNodID) == true)
                {
                    try
                    {
                        int i = int.Parse(textBoxCFG10.Text);
                        int index = TempBoards.IndexOf(CurTempNodID);
                        ((TemperatueBoard)TempBoards[index]).SetConfig(10, i);
                    }
                    catch
                    {
                        MessageBox.Show("Error: wrong input. Please input an integer!");
                        int index = TempBoards.IndexOf(CurTempNodID);
                        int[] config = ((TemperatueBoard)TempBoards[index]).GetConfig();
                        textBoxCFG1.Text = config[9].ToString();
                    }
                }
            }
        }

        private void textBoxCFG11_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                if (TempBoards.Contains(CurTempNodID) == true)
                {
                    try
                    {
                        int i = int.Parse(textBoxCFG11.Text);
                        int index = TempBoards.IndexOf(CurTempNodID);
                        ((TemperatueBoard)TempBoards[index]).SetConfig(11, i);
                    }
                    catch
                    {
                        MessageBox.Show("Error: wrong input. Please input an integer!");
                        int index = TempBoards.IndexOf(CurTempNodID);
                        int[] config = ((TemperatueBoard)TempBoards[index]).GetConfig();
                        textBoxCFG1.Text = config[10].ToString();
                    }
                }
            }
        }

        private void textBoxCFG12_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                if (TempBoards.Contains(CurTempNodID) == true)
                {
                    try
                    {
                        int i = int.Parse(textBoxCFG12.Text);
                        int index = TempBoards.IndexOf(CurTempNodID);
                        ((TemperatueBoard)TempBoards[index]).SetConfig(12, i);
                    }
                    catch
                    {
                        MessageBox.Show("Error: wrong input. Please input an integer!");
                        int index = TempBoards.IndexOf(CurTempNodID);
                        int[] config = ((TemperatueBoard)TempBoards[index]).GetConfig();
                        textBoxCFG1.Text = config[11].ToString();
                    }
                }
            }
        }

        private void textBoxCFG13_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                if (TempBoards.Contains(CurTempNodID) == true)
                {
                    try
                    {
                        int i = int.Parse(textBoxCFG13.Text);
                        int index = TempBoards.IndexOf(CurTempNodID);
                        ((TemperatueBoard)TempBoards[index]).SetConfig(13, i);
                    }
                    catch
                    {
                        MessageBox.Show("Error: wrong input. Please input an integer!");
                        int index = TempBoards.IndexOf(CurTempNodID);
                        int[] config = ((TemperatueBoard)TempBoards[index]).GetConfig();
                        textBoxCFG1.Text = config[12].ToString();
                    }
                }
            }
        }

        private void textBoxCFG14_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                if (TempBoards.Contains(CurTempNodID) == true)
                {
                    try
                    {
                        int i = int.Parse(textBoxCFG14.Text);
                        int index = TempBoards.IndexOf(CurTempNodID);
                        ((TemperatueBoard)TempBoards[index]).SetConfig(14, i);
                    }
                    catch
                    {
                        MessageBox.Show("Error: wrong input. Please input an integer!");
                        int index = TempBoards.IndexOf(CurTempNodID);
                        int[] config = ((TemperatueBoard)TempBoards[index]).GetConfig();
                        textBoxCFG1.Text = config[13].ToString();
                    }
                }
            }
        }

        private void textBoxCFG15_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                if (TempBoards.Contains(CurTempNodID) == true)
                {
                    try
                    {
                        int i = int.Parse(textBoxCFG15.Text);
                        int index = TempBoards.IndexOf(CurTempNodID);
                        ((TemperatueBoard)TempBoards[index]).SetConfig(15, i);
                    }
                    catch
                    {
                        MessageBox.Show("Error: wrong input. Please input an integer!");
                        int index = TempBoards.IndexOf(CurTempNodID);
                        int[] config = ((TemperatueBoard)TempBoards[index]).GetConfig();
                        textBoxCFG1.Text = config[14].ToString();
                    }
                }
            }
        }

        private void textBoxCFG16_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                if (TempBoards.Contains(CurTempNodID) == true)
                {
                    try
                    {
                        int i = int.Parse(textBoxCFG16.Text);
                        int index = TempBoards.IndexOf(CurTempNodID);
                        ((TemperatueBoard)TempBoards[index]).SetConfig(16, i);
                    }
                    catch
                    {
                        MessageBox.Show("Error: wrong input. Please input an integer!");
                        int index = TempBoards.IndexOf(CurTempNodID);
                        int[] config = ((TemperatueBoard)TempBoards[index]).GetConfig();
                        textBoxCFG1.Text = config[15].ToString();
                    }
                }
            }
        }  
