    ...
    public partial class Form1 : Form
    {
        ...
        private void Display(UInt16 nodeID, UInt16 type)
        {
            ...
            switch (type)
            {
                ...
                */
                case 0x0400:
                    if (nodeID == CurTelosbID)
                    {                 
                        i = TelosbSensors.IndexOf(nodeID);
                        textBoxTemp.Text = string.Format("{0:0.00}", ((TelosbSensor)TelosbSensors[i]).Temperature);
                        textBoxHumi.Text = string.Format("{0:0.00}", ((TelosbSensor)TelosbSensors[i]).Humidity);
                        textBoxDew.Text = string.Format("{0:0.00}", ((TelosbSensor)TelosbSensors[i]).DewPoint);
                    }
                    break;
                /*
                case 0x0401:
        ...
        {
            CurTelosbID = UInt16.Parse(comboBoxT.SelectedItem.ToString());
            int index = TelosbSensors.IndexOf(CurTelosbID);
            int config = ((TelosbSensor)TelosbSensors[index]).GetConfig();
            textBoxCFG1.Text = config.ToString();            
        }
        ...
        private void buttonSave_Click(object sender, EventArgs e)
        {
            ...
            string dir = Directory.GetCurrentDirectory() + "\\Config\\Telosb.config";
            fs = new FileStream(dir, fileMode, FileAccess.Write);
            sw = new StreamWriter(fs, Encoding.Default);

            string content;
            content = "##this file is the configuration file for the Telosb Sensors boards. Each line is a configuration for one board.\r\n";
            content += "#Each line should follow the following format:\r\n";
            content += "#ID:TEMP\r\n";
            content += "##the ID is the ID of the telosb sensor. TEMP is the temperature configuration for that sensor.\r\n\r\n";

            foreach (object o in Telosbconfig)
            {
                if (TelosbSensors.Contains(((TelosbConfig)o).id) == false)
                {
                    content += ((TelosbConfig)o).id.ToString() + ":" + ((TelosbConfig)o).config.ToString();                    
                    content += "\r\n";
                }
            }
            foreach (object o in TelosbSensors)
            {
                content += ((TelosbSensor)o).ID.ToString() + ":" + ((TelosbSensor)o).GetConfig().ToString();                
                content += "\r\n";
            }
        ...
        private void textBoxCFG1_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            {
                if (TelosbSensors.Contains(CurTelosbID) == true)
                {
                    try
                    {
                        int i = int.Parse(textBoxCFG1.Text);
                        int index = TelosbSensors.IndexOf(CurTelosbID);
                        ((TelosbSensor)TelosbSensors[index]).SetConfig(i);
                    }
                    catch
                    {
                        MessageBox.Show("Error: wrong input. Please input an integer!");
                        int index = TelosbSensors.IndexOf(CurTelosbID);
                        textBoxCFG1.Text = ((TelosbSensor)TelosbSensors[index]).GetConfig().ToString();                        
                    }
                }
            }
        }  
